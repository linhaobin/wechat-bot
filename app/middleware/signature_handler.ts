...
export default () => {
  return async function signatureHandler(ctx: Context, next) {
    ...
    if (sessionId) {
      await checkHahSession(ctx, { sessionId, timestamp, nonce, signature })
    } else {
      await checkNotHahSession({ timestamp, nonce, signature })
    }
...
async function checkHahSession(ctx: Context, { sessionId, timestamp, nonce, signature }) {
  ...
  const okSignature = genSignature(sessionId, session.accessToken, nonce, timestamp)
...
// 检查有sessionId
function checkNotHahSession({ timestamp, nonce, signature }) {
